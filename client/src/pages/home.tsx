...
// Kart bileşenini ana bileşenden ayıralım (memoizasyon için)
...
export default function Home() {
  ...
  // Hesaplamaları sadece veri değiştiğinde yap
  const { titles, totalPages } = useMemo(() => {
    const titles = data?.titles ?? [];
    const totalPages = Math.ceil((data?.total ?? 0) / limit);
    return { titles, totalPages };
  }, [data, limit]);

  // RequestIdleCallback kullanarak yükleme sonrası ağır hesaplamaları ertele
  useCallback(() => {
    if (titles.length > 0 && 'requestIdleCallback' in window) {
      (window as any).requestIdleCallback(() => {
        // Öncelikli olmayan işlemler buraya (örn. analitik)
        console.log('İçerik yüklendi, boşta kalan zamanda işlemler yapılıyor');
      });
    }
  }, [titles]);
